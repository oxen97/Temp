...
function strandWorldPoint(
  ...
  const x =
    (point.x - centerX) * visual.scaleX * (element.flipX ? -1 : 1);
  const y =
    (point.y - centerY) * visual.scaleY * (element.flipY ? -1 : 1);
  const angle = ((element.rotation + visual.rotate) * Math.PI) / 180;
  return {
    x: element.x + visual.tx + centerX + x * Math.cos(angle) - y * Math.sin(angle),
    y: element.y + visual.ty + centerY + x * Math.sin(angle) + y * Math.cos(angle),
  };
}
  ...
  return {
    x:
      ...
      (x * Math.cos(angle) + y * Math.sin(angle)) /
        (Math.max(0.0001, Math.abs(visual.scaleX)) *
          (element.flipX ? -1 : 1)),
    y:
      centerY +
      (-x * Math.sin(angle) + y * Math.cos(angle)) /
        (Math.max(0.0001, Math.abs(visual.scaleY)) *
          (element.flipY ? -1 : 1)),
  };
}
...
function constrainedStrandGrab(
  ...
  const scale = distance > maxDisplacement
...
function strandBendForElement(
  ...
  const drag =
    interaction.trigger === "pointer-move" && pointer
      ? {
          dx:
            (pointer.x - (element.x + element.width / 2)) *
            pointerInfluence,
          dy:
            (pointer.y - (element.y + element.height / 2)) *
            pointerInfluence,
        }
  ...
  return {
    ...
    focusY: focusPointer
      ? element.height / 2 - focusX * Math.sin(radians) + focusY * Math.cos(radians)
      : undefined,
  };
}
      ...
      for (const [id, { element, interaction, visual }] of inputs) {
        ...
        const localPointer =
          pointer && canReactToPointer
            ...
            : null;
        let grab = active?.elementId === id
          ? constrainedStrandGrab(pose, active.grab, interaction.strandMaxDisplacement)
          : undefined;
          ...
          if (merge && interaction.strandMaxDisplacement > 0) {
            ...
            const closest = movingPose.points.reduce(
              (best, point, index) => {
                ...
                const distance = Math.hypot(world.x - active.x, world.y - active.y);
            ...
            if (latch?.sourceId === active.elementId) {
              ...
              grab = {
                ...
                target: strandLocalPoint(
                  {
                    x: currentWorld.x + (desiredWorld.x - currentWorld.x) * follow,
                    y: currentWorld.y + (desiredWorld.y - currentWorld.y) * follow,
                  },
                  element,
                  visual,
                ),
        ...
        const unsettled = pose.points.some(
          (point, index) =>
            ...
            Math.hypot(pose.velocities[index].x, pose.velocities[index].y) > 0.01,
        );
                  ...
                  return (
                    <div
                      ...
                      onPointerCancel={(event) => {
                        ...
                        if (activeStrandPointerRef.current?.elementId === element.id)
                      ...
                      onPointerDown={(event) => {
                        ...
                        const worldPointer = pageRect
                          ? {
                              x: (event.clientX - pageRect.left) / Math.max(0.0001, layout.scaleX),
                              y: (event.clientY - pageRect.top) / Math.max(0.0001, layout.scaleY),
                            }
                          : null;
                        const strandInteraction = strandInteractionForElement(element);
                        const authoredPath = strandInteraction && strandPathForElement(element);
                        const pose = strandPosesRef.current.get(element.id) ??
                          (strandInteraction && authoredPath
                            ...
                            : undefined);
                        if (pose?.points.length && !strandPosesRef.current.has(element.id)) {
                          const next = new Map(strandPosesRef.current);
                          next.set(element.id, pose);
                          strandPosesRef.current = next;
                          setStrandPoses(next);
                        }
                        const strandGrab = worldPointer && pose?.points.length
                          ? closestStrandBone(
                              pose,
                              strandLocalPoint(worldPointer, element, runtimeVisual),
                            )
                          : undefined;
                      ...
                      onPointerMove={(event) => {
                        ...
                        if (pullsNeighbors && session.strandGrab) {
                          ...
                          if (rect) {
                            ...
                            const local = strandLocalPoint(artboardPointer, element, runtimeVisual);
                            const grab = {
                            ...
                            const interaction = strandInteractionForElement(element);
                            if (pose && interaction) {
                              if (interaction.dragAxis === "horizontal") {
                                grab.target.y = pose.rest[grab.index].y;
                              } else if (interaction.dragAxis === "vertical") {
                              ...
                              const next = new Map(strandPosesRef.current);
                              next.set(element.id, stepStrandPose(pose, {
                                dt: 1 / 60,
                                grab: constrained,
                                maxDisplacement: interaction.strandMaxDisplacement,
                                stiffness: interaction.strandStiffness,
                    ...
                    >
                      {element.type === "text" ? (
                        ...
                        <ShapeGraphic
                          ...
                          strandPathData={strandPose ? strandPosePath(strandPose) : undefined}
